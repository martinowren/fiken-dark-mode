window.addEventListener('load', (e) => {
	...
	const listOfAllTransactions = document.querySelectorAll('.panel.panel-default table tbody tr')
	...
	for (var key in objectOfDates) {
		if (objectOfDates.hasOwnProperty(key)) {
			...
			if(kidArr.length > 0) {
				...
				let sum = 0;
				kidArr.forEach(el => {
					sum += Number(el.cells[2].innerText.replace(",", ".").replace(" ", ""))
				})
				...
				td.colSpan = 100;
				td.innerText = "Summen av dagens kid: " + Math.round(sum * 100) / 100
				tr.appendChild(td)
				if (kidArr[kidArr.length-1].nextSibling) {
					kidArr[kidArr.length-1].parentNode.insertBefore(tr, kidArr[kidArr.length-1].nextSibling);
				}
				else {
